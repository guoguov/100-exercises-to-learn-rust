...
pub fn sum(v: Vec<i32>) -> i32 {
    ...
    let left_slice = left.to_vec();
    let right_slice = right.to_vec();

    let left_handle = thread::spawn(move || left_slice.iter().sum::<i32>());
    let right_handle = thread::spawn(move || right_slice.iter().sum::<i32>());
